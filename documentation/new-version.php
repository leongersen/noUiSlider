...
<section>
	...
	<div class="view">
		...
		<p>Add to that the rapid progress on the Internet Explorer front. Up to noUiSlider 7, IE7 remained supported, mainly because there was never any need to explicity break compatibility. With IE currently at version 11 and Microsoft Edge ('Project Spartan') approaching,</p>

		<p>noUiSlider 8 removes the jQuery dependancy completely.</p>
		...
		<p>I am NOT saying you shouldn't use jQuery: in fact, you probably should.</p>

		<p>If you require help with any specifics, feel free to <a href="mailto:leon.gersen+public@gmail.com">email me</a>. If you do, consider providing a <a href="http://jsfiddle.net/">jsFiddle</a> demonstrating your problem.</p>


		<p>The LibLink project, which I've always liked working on, formed a problem in this port. It's API is completely rooted in jQuery's method system. Considering the number of support request for this featuere, even though it is rather complex, was low, I'm going to assume it wasn't used a lot. I wanted the spirit to remain in place, so I put a lot of thought into a replacement feature. It had to be 1) simpler, 2) have a smaller footprint and 3) be more obvious to use.</p>

		<p>The solution I came up with is the new <code>update</code> event. It fires on any change in the slider value, <strong>and when it is bound</strong>. This allows synchronizing with input elements without calling the sync. function manually. A feature that is missing is the automatic adding of <code>change</code> events to the inputs: I've attempted to compensate this by providing some additional examples.</p>
...
<section>
	...
	<ul>
		...
		<li>Looping and iterating are gone.<br>
		...
		<li>Slider fires no native events anymore.</li>
			<ul>
				...
				<li>The <code>slide</code> event now fires for both handles when dragging</li>
				<li>You can now find the changed handle in the slide event</li>
			</ul>
